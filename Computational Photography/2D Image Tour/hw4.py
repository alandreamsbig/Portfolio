...
def computefaces(corner1, corner2, vanishingpt, imwidth, imheight, focallen):
    ...
    width2d = backright - backleft  
    height2d =  backbottom - backtop
    ...
    topdepth = backdepth - b*backdepth/vanishingpt[1]
     
    back3d = np.array([[backwidth,backheight,0],[0,backheight,0],[0,0,0],[backwidth,0,0]])
    right3d = np.array([[backwidth,backheight,951.2425291],[backwidth,backheight,0],[backwidth,0,0],[backwidth,0,951.2425291]])
    top3d = np.array([[backwidth,backheight,topdepth],[0,backheight,topdepth],[0,backheight,0],[backwidth,backheight,0]])
    left3d = np.array([[0,backheight,0],[0,backheight,884.07681839],[0,0,884.07681839],[0,0,0]])
    bottom3d = np.array([[backwidth,0,0],[0,0,0],[0,0,d],[backwidth,0,d]])
...
def homographywarp(source, homography, imwidth, imheight):
    ...
    for y in xrange(imheight):
        for x in xrange(imwidth):
            ...
            s = np.linalg.solve(homography,[x,y,1])
            s /= s[2]
            s = s.astype(int)
            if(s[0] >=0 and s[0]<imwidth and s[1]>=0 and s[1]<imheight and y < newimage.shape[0] and x < newimage.shape[1] and s[0] < source.shape[1] and s[1] < source.shape[0]):
                newimage[y,x] = source[s[1],s[0]]
