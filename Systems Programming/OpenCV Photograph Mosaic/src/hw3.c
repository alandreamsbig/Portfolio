...
IplImage* stitchImages(IplImage** iclosest, int numColumns, int numRows) {
	...
	CvSize finalsize = cvSize((numColumns*size.width), (numRows*size.height));
	// TODO: allocate the return image. This can be potentially large, so
	// you should make sure the result is not null
	IplImage* finalImage = cvCreateImage(finalsize, iclosest[0]->depth,iclosest[0] -> nChannels);
	...
	if (finalImage != NULL)
	{
		...
		for(i = 0; i < numRows; i++)
		{
			...
			cvSetImageROI(finalImage, cvRect(j*size.width, i*size.height,size.width,size.height));
			// TODO: copy the proper image into the result
			cvCopy(iclosest[count], finalImage, NULL);	
