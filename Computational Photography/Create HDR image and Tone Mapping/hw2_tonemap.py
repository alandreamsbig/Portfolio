...
def bilateral_tonemap(image, debug, sigmaRange, sigmaSpace, contrast, gamma):
    ...
    #logintensity = logintensity[...,np.newaxis]
    #detail = np.subtract(np.repeat(logintensity,3,axis = 2),baseImage)
    ...
    s = np.log(contrast)/ (max_value - min_value)
    contrastReducedBaseImage = (baseImage - max_value)*s
    ...
    colorReconstructedImage = image * (reconst/intensity)[...,np.newaxis]

    # Apply any gamma correction requested
    gammaCorrected = np.power(colorReconstructedImage, 0.5)
    
    # Normalize the results. Do each channel separately.
    gammaCorrected = gammaCorrected - np.amin(gammaCorrected)
    gammaCorrected[:,:,0] = gammaCorrected[:,:,0] / np.amax(gammaCorrected[:,:,0])
    gammaCorrected[:,:,1] = gammaCorrected[:,:,1] / np.amax(gammaCorrected[:,:,1])
    gammaCorrected[:,:,2] = gammaCorrected[:,:,2] / np.amax(gammaCorrected[:,:,2])
